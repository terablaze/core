...
class EventFake implements EventDispatcherInterface
{
    ...
    public function __construct(EventDispatcherInterface $dispatcher, $eventsToFake = [])
    {
        ...
        $this->eventsToFake = ArrayMethods::wrap($eventsToFake);
    }
    ...
    public function except($eventsToDispatch)
    {
        $this->eventsToDispatch = array_merge(
            ...
            ArrayMethods::wrap($eventsToDispatch)
        );
    ...
    public function assertListening($expectedEvent, $expectedListener)
    {
        foreach ($this->dispatcher->getListeners($expectedEvent) as $listenerClosure) {
            ...
            if (is_string($actualListener) && StringMethods::contains($actualListener, '@')) {
                $actualListener = StringMethods::parseCallback($actualListener);

                if (is_string($expectedListener)) {
                    if (StringMethods::contains($expectedListener, '@')) {
                        $expectedListener = StringMethods::parseCallback($expectedListener);
                    } else {
    ...
    {
        $count = count(ArrayMethods::flatten($this->events));
