...
use Terablaze\Bus\QueueingDispatcherInterface;
...
class BusFake implements QueueingDispatcherInterface
{
     ...
     *
     * @var QueueingDispatcherInterface
     */
     ...
     *
     * @param  QueueingDispatcherInterface  $dispatcher
     * @param  array|string  $jobsToFake
     * @return void
     */
    public function __construct(QueueingDispatcherInterface $dispatcher, $jobsToFake = [])
    {
        $this->dispatcher = $dispatcher;
        $this->jobsToFake = ArrayMethods::wrap($jobsToFake);
    ...
    {
        $this->jobsToDispatch = array_merge($this->jobsToDispatch, ArrayMethods::wrap($jobsToDispatch));
...
//        $matching = $this->dispatched($command, $callback)->map->chained->map(function ($chain) {
        $matching = $this->dispatched($command, $callback)->map(function ($chain) {
    ...
    public function dispatched($command, $callback = null)
    {
        ...
        return (new ArrayCollection($this->commands[$command]))->filter(fn ($command) => $callback($command));
    }

    /**
     * Get all the jobs dispatched synchronously matching a truth-test callback.
     *
     * @param  string  $command
     * @param  callable|null  $callback
     * @return CollectionInterface
    ...
    public function dispatchedSync(string $command, $callback = null)
    {
        ...
        return (new ArrayCollection($this->commandsSync[$command]))->filter(fn ($command) => $callback($command));
    }
    ...
    public function dispatchedAfterResponse(string $command, $callback = null)
    {
        ...
        return (new ArrayCollection($this->commandsAfterResponse[$command]))
    ...
    public function batched(callable $callback)
    {
        ...
        return (new ArrayCollection($this->batches))->filter(fn ($batch) => $callback($batch));
    }
    ...
    {
        $jobs = ArrayCollection::wrap($jobs);
    ...
    {
        return new PendingBatchFake($this, ArrayCollection::wrap($jobs));
    }
