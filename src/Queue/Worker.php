...
use Terablaze\Queue\Events\JobReleasedAfterException;
use Terablaze\Queue\Events\Looping;
use Terablaze\Queue\Events\WorkerStopping;
...
class Worker
{
     ...
     *
     * @param QueueFactoryInterface $manager
     ...
     * @param string $queue
     * @param \Terablaze\Queue\WorkerOptions $options
     * @return int
     */
    ...
    protected function daemonShouldRun(WorkerOptions $options, $connectionName, $queue)
    {
        return !((($this->isDownForMaintenance)() && !$options->force) ||
            $this->paused ||
            $this->eventDispatcher->dispatch(new Looping($connectionName, $queue)) === false);
    }
    ...
    protected function handleJobException($connectionName, $job, WorkerOptions $options, Throwable $e)
    {
        try {
            ...
            if (!$job->isDeleted() && !$job->isReleased() && !$job->hasFailed()) {
                ...
                $this->eventDispatcher->dispatch(new JobReleasedAfterException(
                    $connectionName,
                    $job
                ));
            }
    ...
    protected function markJobAsFailedIfWillExceedMaxExceptions($connectionName, $job, Throwable $e)
    {
        ...
        if (!$this->cache->get('job-exceptions:' . $uuid)) {
            $this->cache->set('job-exceptions:' . $uuid, 0, Carbon::now()->addDay());
        }
    ...
    {
        $this->eventDispatcher->dispatch(new JobProcessing(
            $connectionName,
            $job
        ));
    }
    ...
    {
        $this->eventDispatcher->dispatch(new JobProcessed(
            $connectionName,
            $job
        ));
    }
    ...
    {
        $this->eventDispatcher->dispatch(new JobExceptionOccurred(
            $connectionName,
            $job,
            $e
        ));
    ...
    {
        if ($this->cache) {
            return $this->cache->get('terablaze:queue:restart');
        }
        return null;
    }
    ...
    {
        $this->eventDispatcher->dispatch(new WorkerStopping($status, $options));
     ...
     *
     * @param int $status
     * @param \Terablaze\Queue\WorkerOptions|null $options
     * @return never
     */
     ...
     *
     * @param JobInterface $job
     * @return \Terablaze\Queue\Exception\MaxAttemptsExceededException
     */
