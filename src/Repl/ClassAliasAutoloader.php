...
class ClassAliasAutoloader
{
    ...
    public function __construct(Shell $shell, $classMapPath, array $includedAliases = [], array $excludedAliases = [])
    {
        ...
        $this->includedAliases = new ArrayCollection($includedAliases);
        $this->excludedAliases = new ArrayCollection($excludedAliases);
    ...
    {
        if (StringMethods::contains($class, '\\')) {
            return;
        }
        ...
        if ($fullName) {
            $this->shell->writeStdout("[!] Aliasing '{$class}' to '{$fullName}' for this Repl session.\n");
    ...
    public function isAliasable($class, $path)
    {
        ...
        if (! $this->includedAliases->filter(function ($alias) use ($class) {
            return StringMethods::startsWith($class, $alias);
        })->isEmpty()) {
            return true;
        }

        if (StringMethods::startsWith($path, $this->vendorPath)) {
            return false;
        }
