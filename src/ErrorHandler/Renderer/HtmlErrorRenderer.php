...
class HtmlErrorRenderer
{
    ...
    private static $templateDir = 'views/';
    ...
    public function __construct($debug = false, string $charset = null, $fileLinkFormat = null, string $projectDir = null, $outputBuffer = '', LoggerInterface $logger = null)
    {
        if (!\is_bool($debug) && !\is_callable($debug)) {
            throw new \TypeError(sprintf('ArgumentException 1 passed to "%s()" must be a boolean or a callable, "%s" given.', __METHOD__, get_debug_type($debug)));
        }

        if (!\is_string($outputBuffer) && !\is_callable($outputBuffer)) {
            throw new \TypeError(sprintf('ArgumentException 5 passed to "%s()" must be a string or a callable, "%s" given.', __METHOD__, get_debug_type($outputBuffer)));
        }
    ...
    public static function isDebug(Request $request, bool $debug): \Closure
    {
        return static function () use ($request, $debug): bool {
            return $debug && $request->getAttribute('showException') == true;
        };
    }

    public static function getAndCleanOutputBuffer(Request $request): \Closure
    {
        return static function () use ($request): string {
            $startObLevel = $request->getHeaderLine('X-Php-Ob-Level');
            $startObLevel = empty($startObLevel) ? -1 : $startObLevel;
    ...
    private function renderException(
        ...
        string $debugTemplate = __DIR__ . '/../Resources/views/exception.debug.html.php'
    ): string {
        ...
        if (!$debug) {
            ...
            if (is_file($viewFile = self::$templateDir . '/' . $statusCode . '.html.php')) {
                return $this->include($viewFile, $viewData);
            }
            if (is_file($viewFile = self::$templateDir . '/' . $statusCode . '.php')) {
                return $this->include($viewFile, $viewData);
            }
            if (is_file($viewFile = self::$templateDir . '/generic.html.php')) {
                return $this->include($viewFile, $viewData);
            }
            if (is_file($viewFile = self::$templateDir . '/generic.php')) {
                return $this->include($viewFile, $viewData);
            }
            $errorViewFile = 'views/error.html.php';
            return $this->include($errorViewFile, $viewData);
        }
        ...
        return $this->include($debugTemplate, [
            ...
            'statusText' => $statusText,
            'statusCode' => $statusCode,
            'logger' => $this->logger instanceof LoggerInterface ? $this->logger : null,
    ...
    /**
     * Allows overriding the default non-debug template dir.
     *
     * @param string $templateDir path to the custom template directory containing template files
     */
