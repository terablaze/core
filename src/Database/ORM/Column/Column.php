...
class Column
{
    ...
    public function getColumn($property)
    {
        ...
        if (in_array(mb_strtolower($type), ["bool", "boolean"])) {
            $default = (in_array(mb_strtolower($defaultValue), LegacyModel::falsy, true)) ? 0 : 1;
        } else {
        ...
        $nullable = in_array(mb_strtolower($nullableValue), LegacyModel::falsy, true) ? false : true;
        $autoConvertValue = $column['autoconvert'] ?? $this->getFirst($this->propertyMeta, '@autoconvert') ?? true;
        $autoConvert = in_array(mb_strtolower($autoConvertValue), LegacyModel::falsy, true) ? false : true;
