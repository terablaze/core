...
use Closure;
use Terablaze\Database\Exception\DeadlockException;
...
trait ManagesTransactions
{
    ...
    protected function handleTransactionException(Throwable $e, $currentAttempt, $maxAttempts)
    {
        ...
        // If there was an exception we will roll back this transaction, and then we
    ...
    protected function createTransaction()
    {
        ...
        } elseif ($this->transactions >= 1 && $this->query()->supportsSavepoints()) {
    ...
    {
        $this->pdo()->exec(
            $this->query()->compileSavepoint('trans'.($this->transactions + 1))
        );
    }
    ...
    protected function performRollBack($toLevel)
    {
        if ($toLevel == 0) {
            $this->pdo()->rollBack();
        } elseif ($this->query()->supportsSavepoints()) {
            $this->pdo()->exec(
                $this->query()->compileSavepointRollBack('trans'.($toLevel + 1))
            );
        }
    }
