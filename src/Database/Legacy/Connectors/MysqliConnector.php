...
use TeraBlaze\Database\Legacy\Query\MysqliQuery;
...
{
    public const QUERY_BEFORE_EVENT = "terablaze.database.connector.mysql.query.before",
        QUERY_AFTER_EVENT = "terablaze.database.connector.mysql.query.after";
        ...
        if (!empty($query = $model->retrieveSyncSQL())) {
            ...
            foreach ($columns as $column) {
                ...
                switch ($type) {
                    ...
                    case LegacyModel::DATA_TYPES['integer']:
                    {
                        $lines[] = "`{$name}` int(11){$nullable}{$default}";
                        break;
                    }
                    case LegacyModel::DATA_TYPES['decimal']:
                    {
                        $lines[] = "`{$name}` float{$nullable}{$default}";
                        break;
                    }
                    case LegacyModel::DATA_TYPES['boolean']:
                    case LegacyModel::DATA_TYPES['bool']:
                    {
                        $lines[] = "`{$name}` tinyint(1){$nullable}{$default}";
                        break;
                    }
                    case LegacyModel::DATA_TYPES['datetime']:
                    {
