...
#[AsCommand(name: 'schedule:list', description: 'List the scheduled commands')]
class ScheduleListCommand extends Command
{
     ...
     * @var string
     */
    protected static $defaultDescription = 'List the scheduled commands';
    ...
    public function handle(Schedule $schedule)
    {
        ...
        if ($events->isEmpty()) {
            $this->io->info('No scheduled tasks have been defined.');
        ...
        $timezone = new DateTimeZone($this->getOption('timezone') ?? getConfig('app.timezone'));
        ...
        $events = $events->map(function ($event) use ($terminalWidth, $expressionSpacing, $timezone) {
            ...
            if (! $this->output->isVerbose()) {
                $command = str_replace([Application::phpBinary(), Application::blazeBinary()], [
                    'php',
                    preg_replace("#['\"]#", '', Application::blazeBinary()),
                ], $event->command);
            }
    ...
    private function getClosureLocation(CallbackEvent $event)
    {
        ...
        if ($callback instanceof Closure) {
            ...
            return sprintf(
                '%s:%s',
                str_replace($this->kernel->getProjectDir().DIRECTORY_SEPARATOR, '', $function->getFileName() ?: ''),
    ...
    {
        return [
            ['timezone', null, InputOption::VALUE_OPTIONAL, 'The timezone that times should be displayed in'],
        ];
    }
}
