...
class NotificationMakeCommand extends GeneratorCommand
{
     ...
     * @var string
     */
    protected static $defaultDescription = 'Create a new notification class';
    ...
    {
        if (parent::handle() === false && ! $this->getOption('force')) {
            return;
        }

        if ($this->getOption('markdown')) {
    ...
    {
        $path = $this->viewPath(
            str_replace('.', '/', $this->getOption('markdown')).'.pha'
        );
    ...
    protected function buildClass($name)
    {
        ...
        if ($this->getOption('markdown')) {
            $class = str_replace(['DummyView', '{{ view }}'], $this->getOption('markdown'), $class);
        }
    ...
    {
        return $this->getOption('markdown')
