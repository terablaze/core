...
class MailMakeCommand extends GeneratorCommand
{
     ...
     * @var string
     */
    protected static $defaultDescription = 'Create a new email class';
    ...
    {
        if (parent::handle() === false && ! $this->getInput()->getOption('force')) {
            return;
        }

        if ($this->getInput()->getOption('markdown') !== false) {
    ...
    protected function writeMarkdownTemplate()
    {
        ...
        $this->files->put($path, file_get_contents(dirname(__DIR__).'/stubs/markdown.stub'));
    }
    ...
    protected function buildClass($name)
    {
        $class = str_replace(
            '{{ subject }}',
            StringMethods::headline(str_replace($this->getNamespace($name).'\\', '', $name)),
    ...
    {
        $view = $this->getInput()->getOption('markdown');

        if (! $view) {
            $name = str_replace('\\', '/', $this->getInput()->getArgument('name'));

            $view = 'mail.'.collect(explode('/', $name))
                ->map(fn ($part) => StringMethods::kebab($part))
                ->implode('.');
        }
    ...
    protected function getStub()
    {
        return $this->resolveStubPath(
            $this->getInput()->getOption('markdown') !== false
